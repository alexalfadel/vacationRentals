...
function NewSpotForm() {
  ...
  useEffect(() => {
    ...
    if (!validImage(image4) && image4.length)
      error.image4 = "Image URL must end in .png, .jpg, or .jpeg";
    if (!validImage(image5) && image5.length)
      error.image5 = "Image URL must end in .png, .jpg, or .jpeg";
  ...
  return (
    <div className="new-spot-form-box">
      ...
      <p className="new-spot-header-p">
        Guests will only get your exact address once they booked a reservation.
      </p>
      <form className="new-spot-form" onSubmit={onSubmit}>
        <div className="wide-input-box">
          {/* {showErr && <p>{errors.country}</p>} */}
          <label className="new-spot-input-labels" htmlFor="country">
            Country{" "}
            <span>
              {showErr && <p className="new-spot-errors">{errors.country}</p>}
            </span>
          </label>
          <input
            placeholder="Country"
        ...
        <div>
          ...
          <label className="new-spot-input-labels" htmlFor="address">
            Street Address
            <span>
              {showErr && <p className="new-spot-errors">{errors.address}</p>}
            </span>
          </label>
          <input
            placeholder="Address"
        ...
        <div className="city-state-box">
          <div className="city-box">
            ...
            <label className="city-label" htmlFor="city">
              City
              <span>
                {showErr && <p className="new-spot-errors">{errors.city}</p>}
              </span>
            </label>
            <input
              placeholder="City"
          ...
          <div className="state-box">
            ...
            <label className="state-label" htmlFor="state">
              State
              <span>
                {showErr && <p className="new-spot-errors">{errors.state}</p>}
              </span>
            </label>
            <input
              placeholder="State"
        ...
        <div className="lat-lng-box">
          <div className="lat-box">
            ...
            <label className="lat-label" htmlFor="lat">
              Latitude
              <span>
                {showErr && <p className="new-spot-errors">{errors.lat}</p>}
              </span>
            </label>
            <input
          ...
          <div className="lng-box">
            ...
            <label className="lng-label" htmlFor="lng">
              Longitude
              <span>
                {showErr && <p className="new-spot-errors">{errors.lng}</p>}
              </span>
            </label>
            <input
